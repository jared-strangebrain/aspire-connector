...
function pick<T>(...vals: T[]) { return vals.find((v) => v !== undefined && v !== null); }

export const tokenRequestHandler = OperationHandlerSetup
  configure("token_request", (handler: any) =>
    handler.usingComposite(async (ctx: any, input: TokenRequestInput) => {
      ...
      const body = shouldRefresh ? { RefreshToken: current!.refresh_token }
      ...
      const access_token = pick(data.accessToken, data.AccessToken, data.token, data.Token, data.jwt, data.JWT);
      if (!access_token) throw new Error("Aspire auth response missing access token");

      const refresh_token = pick(data.refreshToken, data.RefreshToken, data.refresh_token, data.Refresh_token, current?.refresh_token);
      const expires_in = pick<number>(data.expiresIn, data.ExpiresIn, data.expires_in);
      const expires_at = typeof expires_in === "number" ? new Date(Date.now() + expires_in * 1000).toISOString() : current?.expires_at;
