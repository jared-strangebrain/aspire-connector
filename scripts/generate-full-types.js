...
console.log('Generating full TypeScript interfaces for all operations...\n');
...
// Generate complete interface from schema
function generateInterface(schema, typeName, depth = 0) {
  ...
  // No properties - might be a simple type
  else {
    return null;
  }
...
function convertPropertyType(schema) {
  ...
  // Handle $ref - use any[] for nested arrays to avoid circular refs
  if (schema.$ref) {
    return 'any';
  }
...
for (const op of operations) {
  try {
    ...
    successCount++;
    const hasDetailed = outputContent.length > 100;
    if (hasDetailed) {
      detailedCount++;
      console.log(`✅ Generated detailed output for ${op.name}`);
    } else {
      simpleCount++;
      console.log(`✅ Generated simple output for ${op.name}`);
    }
  } catch (error) {
    failCount++;
...
console.log(`  📋 Detailed Types: ${detailedCount}`);
console.log(`  📝 Simple Types: ${simpleCount}`);
