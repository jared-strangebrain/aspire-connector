...
console.log(`Fixing array wrapping with correct handleResponse signature...\n`);
...
for (const opName of operations) {
  ...
  // Use the single-parameter form of handleResponse
  const oldPattern = /\.handleResponse\(\(ctx, input, response\) => \{[\s\S]*?return httpResponseBody;\s*\}\)/;
  
  const newCode = `.handleResponse((response) => {
...
console.log('\nHandlers now use correct handleResponse signature');
