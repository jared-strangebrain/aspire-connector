...
import { tags_getHandler } from './handler';

/**
 * Test suite for tags_get
 * 
 * HTTP Method: GET
 * Endpoint: /Tags
 * 
 ...
 */
describe('tags_get', () => {
  OperationHandlerTestSetup.configureHandlerTest(tags_getHandler, (handlerTest) =>
    handlerTest
      ...
      .testCase('should succeed with minimal required inputs', (testCase) =>
        testCase
          ...
          .then(({ output }) => {
            ...
            if (output.isSuccess) {
              // Output is properly typed as TagsGetOutput
