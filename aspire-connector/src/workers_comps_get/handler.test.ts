...
import { workers_comps_getHandler } from './handler';

/**
 * Test suite for workers_comps_get
 * 
 * HTTP Method: GET
 ...
 */
describe('workers_comps_get', () => {
  OperationHandlerTestSetup.configureHandlerTest(workers_comps_getHandler, (handlerTest) =>
    handlerTest
      ...
      .testCase('should succeed with minimal required inputs', (testCase) =>
        testCase
          ...
          .then(({ output }) => {
            ...
            if (output.isSuccess) {
              // Output is properly typed as WorkersCompsGetOutput
