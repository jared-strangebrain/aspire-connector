...
import { receipts_receive_receiptHandler } from './handler';

/**
 * Test suite for receipts_receive_receipt
 * 
 * HTTP Method: POST
 * Endpoint: /Receipts/Receive
 * 
 * REQUIRED INPUTS:
 *   - body (request body)
 * 
 ...
 */
describe('receipts_receive_receipt', () => {
  OperationHandlerTestSetup.configureHandlerTest(receipts_receive_receiptHandler, (handlerTest) =>
    handlerTest
      ...
      .testCase('should succeed with minimal required inputs', (testCase) =>
        testCase
          ...
          .then(({ output }) => {
            ...
            if (output.isSuccess) {
              // Output is properly typed as ReceiptsReceiveReceiptOutput
