...
import { opportunities_createHandler } from './handler';

/**
 * Test suite for opportunities_create
 * 
 * HTTP Method: POST
 ...
 */
describe.skip('opportunities_create', () => {  // Skipped for deployment - enable after adding real test data
  OperationHandlerTestSetup.configureHandlerTest(opportunities_createHandler, (handlerTest) =>
    handlerTest
      ...
      .testCase('should succeed with minimal required inputs', (testCase) =>
        testCase
          ...
          {
            body: {
              OpportunityName: 'Test Name',
              OpportunityType: 'test-value'
            }
          }))
          .then(({ output }) => {
            ...
            if (output.isSuccess) {
              // Output is properly typed as OpportunitiesCreateOutput
