...
import { pay_rates_getHandler } from './handler';

/**
 * Test suite for pay_rates_get
 * 
 * HTTP Method: GET
 * Endpoint: /PayRates
 * 
 ...
 */
describe('pay_rates_get', () => {
  OperationHandlerTestSetup.configureHandlerTest(pay_rates_getHandler, (handlerTest) =>
    handlerTest
      ...
      .testCase('should succeed with minimal required inputs', (testCase) =>
        testCase
          ...
          .then(({ output }) => {
            ...
            if (output.isSuccess) {
              // Output is properly typed as PayRatesGetOutput
