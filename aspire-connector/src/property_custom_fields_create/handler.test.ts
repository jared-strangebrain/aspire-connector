...
import { property_custom_fields_createHandler } from './handler';

/**
 * Test suite for property_custom_fields_create
 * 
 * HTTP Method: POST
 * Endpoint: /PropertyCustomFields
 * 
 * REQUIRED INPUTS:
 *   - body (Request body)
 * 
 ...
 */
describe.skip('property_custom_fields_create', () => {  // Skipped for deployment - enable after adding real test data
  OperationHandlerTestSetup.configureHandlerTest(property_custom_fields_createHandler, (handlerTest) =>
    handlerTest
      ...
      .testCase('should succeed with minimal required inputs', (testCase) =>
        testCase
          ...
          {
            body: {
              PropertyCustomFieldDefinitionID: 1,
              PropertyID: 1
            }
          }))
          .then(({ output }) => {
            ...
            if (output.isSuccess) {
              // Output is properly typed as PropertyCustomFieldsCreateOutput
