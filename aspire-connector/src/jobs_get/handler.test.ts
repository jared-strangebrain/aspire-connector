...
import { jobs_getHandler } from './handler';

/**
 * Test suite for jobs_get
 * 
 * HTTP Method: GET
 * Endpoint: /Jobs
 * 
 ...
 */
describe('jobs_get', () => {
  OperationHandlerTestSetup.configureHandlerTest(jobs_getHandler, (handlerTest) =>
    handlerTest
      ...
      .testCase('should succeed with minimal required inputs', (testCase) =>
        testCase
          ...
          .then(({ output }) => {
            ...
            if (output.isSuccess) {
              // Output is properly typed as JobsGetOutput
